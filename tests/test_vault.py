...
class TestWriteEntry:
    ...
    def test_quoted_wikilink_in_yaml(self, cfg: Config):
        ...
        path = write_entry(cfg, plan, content)
        text = path.read_text()
        # source wikilink should be quoted in YAML (single or double)
        assert "source: '[[test-entry]]'" in text or 'source: "[[test-entry]]"' in text
...
class TestArchiveInbox:
    ...
    def test_archives_plain_url_file(self, cfg: Config):
        import hashlib
        ...
        archived = archive_inbox(cfg, {url_hash})

        assert archived == 1
        assert not plain.exists()
        assert (cfg.archive_dir / "plain.url").exists()
