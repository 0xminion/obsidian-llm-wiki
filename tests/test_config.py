...
class TestLoadConfig:
    ...
    def test_loads_env_from_explicit_vault(self, tmp_path, monkeypatch):
        vault = tmp_path / "CustomVault"
        env_file = vault / "Meta" / "Scripts" / ".env"
        env_file.parent.mkdir(parents=True)
        env_file.write_text("AGENT_CMD=custom-agent\nPARALLEL=9\n", encoding="utf-8")
        ...
        assert cfg.agent_cmd == "custom-agent"
        assert cfg.parallel == 9
        assert os.environ.get("AGENT_CMD") is None
        assert os.environ.get("PARALLEL") is None
