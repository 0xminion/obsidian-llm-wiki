...
from pipeline.compile import _build_edges, _load_prompt, run_compile, CompileResult, VaultSnapshot, _parse_agent_metrics
...
class TestBuildEdgesRegression:
    def test_rebuild_is_idempotent_for_symmetric_links(self, cfg):
        (cfg.entries_dir / "z.md").write_text("---\ntitle: Z\n---\n# Z\n\n[[a]]\n", encoding="utf-8")
        (cfg.entries_dir / "a.md").write_text("---\ntitle: A\n---\n# A\n\n[[z]]\n", encoding="utf-8")

        first = _build_edges(cfg)
        second = _build_edges(cfg)

        lines = cfg.edges_file.read_text(encoding="utf-8").strip().splitlines()
        assert first == 1
        assert second == 0
        assert lines.count("a\tz\trelates_to\tauto-detected wikilink") == 1

    def test_shared_sources_do_not_count_as_shared_tags(self, cfg):
        (cfg.entries_dir / "entry1.md").write_text("---\ntitle: Entry1\n---\n# Entry1\n", encoding="utf-8")
        concept_a = """---
title: Concept A
type: concept
status: draft
...
# Concept A
"""
        concept_b = concept_a.replace("Concept A", "Concept B")
        (cfg.concepts_dir / "concept-a.md").write_text(concept_a, encoding="utf-8")
        (cfg.concepts_dir / "concept-b.md").write_text(concept_b, encoding="utf-8")

        _build_edges(cfg)

        edges = cfg.edges_file.read_text(encoding="utf-8")
        assert "shared tags:" not in edges
