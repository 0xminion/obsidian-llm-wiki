...
class TaskModelConfig:
    ...
    def __post_init__(self) -> None:
        ...
        for field_name in ("ingest_model", "maintenance_model", "query_model"):
            override = getattr(self, field_name)
            if override is not None and not override.strip():
                raise ValueError(f"{field_name} must not be blank when provided")


_SUPPORTED_TASKS = frozenset({"ingest", "maintenance", "query"})
...
def resolve_task_model(config: TaskModelConfig, task: str) -> str:
    ...
    if task not in _SUPPORTED_TASKS:
        raise ValueError(f"Unsupported task: {task!r}. Supported: ingest, maintenance, query.")
