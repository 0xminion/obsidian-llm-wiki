...
def _strip_xml_ns(elem):
    """Recursively strip XML namespaces from element tags for uniform lookup."""
...
def _iter_items(root):
    """Yield all <item> elements from an RSS root, namespace-agnostic."""
    for elem in root.iter():
        if hasattr(elem, 'tag') and isinstance(elem.tag, str) and elem.tag == "item":
            yield elem


def _find_child(item, tag: str):
    """Find first child element with given local tag name, namespace-agnostic."""
    for child in item:
...
def _parse_rss_episode(feed_url: str, episode_id: str, episode_slug: str = "",
    ...
    if target_item is None and episode_slug:
        ...
        if best_score >= 0.5 and best_item is not None:
            ...
            log.info("RSS: matched episode by title (score=%.2f): %s",
                     best_score,
                     (_find_child(best_item, "title").text if _find_child(best_item, "title") is not None else "?"))
    ...
    if target_item is None and items:
        ...
        log.warning("RSS: no episode match found, falling back to latest episode: %s",
                    (_find_child(target_item, "title").text if _find_child(target_item, "title") is not None else "?"))

    if target_item is None:
        return "", "", ""

    enclosure = _find_child(target_item, "enclosure")
    audio_url = enclosure.get("url", "") if enclosure is not None else ""

    desc_elem = _find_child(target_item, "description")
    description = (desc_elem.text or "")[:5000] if desc_elem is not None else ""

    title_elem = _find_child(target_item, "title")
