...
def setup_git_hooks(vault_path: Path) -> None:
    ...
    if not git_dir.is_dir():
        ...
        print("Initializing...")
        _run(["git", "init"], timeout=30)
        gitignore = vault_path / ".gitignore"
        gitignore.write_text("# Wiki Vault\n.DS_Store\n*.tmp\n")
        _run(["git", "add", "-A"])
        _run(["git", "commit", "-m", "Initial vault commit", "--quiet"])
    ...
    diff_result = _run(["git", "diff", "--quiet"])
    cached_diff_result = _run(["git", "diff", "--cached", "--quiet"])
