...
def check_dependencies(
    ...
    Args:
        required: List of required command names. Defaults to REQUIRED_TOOLS.
        optional: List of optional command names. Defaults to OPTIONAL_TOOLS.

    Returns:
        Tuple of (missing_required, missing_optional) command name lists.
    """
    ...
    if missing_required:
        log.error("Missing required dependencies: %s", ", ".join(missing_required))

    if missing_optional:
        log.warning("Missing optional dependencies: %s", ", ".join(missing_optional))
